...
class Phacker.Game.Sprite
    ...
    constructor: (@gm, @waterliliesO) -> # wls is array of waterlily obj
        ...
        @anim_jump = @spt.animations.add 'jmp', [1, 2, 1, 0, 3], 20, false
        #@anim_jump.onComplete.add( @turn, @ )

        @anim_down = @spt.animations.add 'dwn', [0, 1, 2, 1, 0], 20, false
        @anim_down.onComplete.add(
            @turn #@spt.angle = 20 #@go_center_lily.start();console.log "- #{@_fle_} : ",'fc'
            @
        )
    ...
    #.----------.----------
    check_height: (spt) ->
        if spt.y > @glob.spt.max_height  then return 'loose'
        else return 'ok'
    ...
    #----------.----------
    turn: () ->
        if  @wls[1].prm.way is 'left' then     @spt.scale.setTo   .8,  .8;    @spt.angle =  20
        else                                   @spt.scale.setTo   -.8, .8;    @spt.angle = -20
